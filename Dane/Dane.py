...
# Przykład użycia funkcji w bezpieczny sposób
def test_wczytaj_csv(sciezka_pliku: str, separator: str = None, format_daty: str = None) -> pd.DataFrame:
    """
    Funkcja testowa wczytująca CSV z obsługą błędów
    ...
    """
    try:
        print(f"\n[INFO] Testowe wczytywanie pliku {os.path.basename(sciezka_pliku)}...")

        # Sprawdź czy plik istnieje
        if not os.path.exists(sciezka_pliku):
            print(f"[BŁĄD] Plik nie istnieje: {sciezka_pliku}")
            return None
        ...
        print(f"[BŁĄD] Nieoczekiwany błąd: {str(e)}")
        import traceback
        print(traceback.format_exc())
        return None
...
if __name__ == "__main__":
    # Przykładowe ścieżki (zmień na właściwe)
    sciezka_retail = "online_retail_II.csv"
    sciezka_student = "student-mat.csv"

    # Sprawdź istnienie plików
    if os.path.exists(sciezka_retail):
        test_wczytaj_csv(sciezka_retail)
    else:
        print(f"[BŁĄD] Plik {sciezka_retail} nie istnieje")

    if os.path.exists(sciezka_student):
        test_wczytaj_csv(sciezka_student)
    else:
        print(f"[BŁĄD] Plik {sciezka_student} nie istnieje")